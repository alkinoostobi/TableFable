...
export const combatStore = defineStore("combat", {
  ...
  actions: {
    ...
    targetSelected() { },
    attack(charID, attackIndex) {
      this.action = 'attack'
      let category = tokenInfo.getCategoryById(charID)
      let attackobj = tokenInfo.tokens[category].charID.attack[attackIndex]
      this.loadedAttack.range = attackobj.range
      this.loadedAttack.damage = attackobj.damage
      this.loadedAttack.bonus = attackobj.bonus
      this.loadedAttack.targets = attackobj.targets
    },
    spell() { },
    ability() { },
