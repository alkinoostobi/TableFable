  ...
  actions: {
    ...
    canTokensReach(tokenA, tokenB, distance) {
      ...
      const middleA = [
        ...
        {
          x: leftTopA.x + sizetranslate.feetTranslator(sizeA.x) / 2,
          y: leftTopA.y + sizetranslate.feetTranslator(sizeA.y)
        },
        {x: leftTopA.x, y: leftTopA.y + sizetranslate.feetTranslator(sizeA.y) / 2},
        {
          x: leftTopA.x + sizetranslate.feetTranslator(sizeA.x),
          y: leftTopA.y + sizetranslate.feetTranslator(sizeA.y) / 2
        }
      ];
      ...
      const middleB = [
        ...
        {
          x: leftTopB.x + sizetranslate.feetTranslator(sizeB.x) / 2,
          y: leftTopB.y + sizetranslate.feetTranslator(sizeB.y)
        },
        {x: leftTopB.x, y: leftTopB.y + sizetranslate.feetTranslator(sizeB.y) / 2},
        {
          x: leftTopB.x + sizetranslate.feetTranslator(sizeB.x),
          y: leftTopB.y + sizetranslate.feetTranslator(sizeB.y) / 2
        }
      ];
